...
//Register related:
#define W_REG_COMMAND_HEAD 0x20
#define R_REG_COMMAND_HEAD 0x00
#define NOP 0xFF
#define EN_AA_REG_ADDR 0x01
#define CONFIG_REG_ADDR 0x00
#define RF_CH_REG_ADDR 0x05

#define MAX_FREQ 2525
...
void nRF24L01P::setRegister(int regAddress, int regData) {
    ...
    unsigned char command = (W_REG_COMMAND_HEAD) | static_cast<unsigned char>(regAddress);
    int ceVal = ce_;
    ce_ = 0;
    wait_us(_NRF24L01P_TIMING_Thce_us);
    nCS_ = 0;
    spi_.write(command);
    spi_.write(static_cast<unsigned char>(regData));
    nCS_ = 1;
    wait_us(_NRF24L01P_TIMING_Thce_us);
    ce_ = ceVal;
...
int nRF24L01P::getRegister(int regAddress) {
	...
	// The use the spi_, nCS_, and ce_ member variables included in the nRF24L01P class.
	// Only use DigitalOut & SPI MBed functions when implementing this function.
    unsigned char command  = (R_REG_COMMAND_HEAD) | static_cast<unsigned char>(regAddress);
    int ceVal = ce_;
    ce_ = 0;
    wait_us(_NRF24L01P_TIMING_Thce_us);
    nCS_ = 0;
	spi_.write(command);
    unsigned char regData = spi_.write(static_cast<unsigned char>(NOP));
    nCS_ = 1; 
    wait_us(_NRF24L01P_TIMING_Thce_us);
    ce_ = ceVal;
    return regData;
...
void nRF24L01P::enableAutoAcknowledge(int pipe) {
	...
	// This function either enables the bit corresponding to the specified pipe in the Enable AutoAck register.
    if(pipe < 0 || pipe > 5) return;
    unsigned char enAA_bit = 0b1 << pipe;
    unsigned char enAA = getRegister(EN_AA_REG_ADDR);
    setRegister(EN_AA_REG_ADDR, (enAA | enAA_bit));
...
void nRF24L01P::setRfFrequency(int frequency) {
    ...
    if(frequency > MAX_FREQ || frequency < MIN_FREQ) return;
    else setRegister(RF_CH_REG_ADDR, static_cast<unsigned char>(frequency - MIN_FREQ));
...
void nRF24L01P::setCrcWidth(int width) {
    ...
    if(width != 0 && width != 8 && width != 16)
        ...
        unsigned char config = getRegister(CONFIG_REG_ADDR);
        if(width == 0)
            config &= 0xF7; //EN_CRC bit low
        else
            config |= 0x08; //EN_CRC bit high

        if(width == 8)
            config &= 0xFB; //CRCO bit low 
        else if(width == 16)
            config |= 0x04; //CRCO bit high
        setRegister(CONFIG_REG_ADDR, config);
    }
